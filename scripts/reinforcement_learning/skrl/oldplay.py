...
parser.add_argument("--log_data", action="store_true", default=True, help="Log data during play.")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, experiment_cfg: dict):
    ...
    def _extract_all_world_positions():
        """Return flattened list of [r0_x, r0_y, r0_z, r1_x, r1_y, r1_z, ...] for env 0.
        Empty list for single-robot envs or on failure."""
        import torch as _torch
        try:
            unwrapped = env.unwrapped
            if hasattr(unwrapped, "_robots"):
                states = [r.data.root_state_w for r in unwrapped._robots]  # each [num_envs, 13]
                pos_list = []
                for s in states:
                    # s[0, :3] is (x, y, z) for env 0
                    x = float(s[0, 0].item())
                    y = float(s[0, 1].item())
                    z = float(s[0, 2].item())
                    pos_list.extend([x, y, z])
                return pos_list
        except Exception:
            pass
        return []
    ...
    if args_cli.log_data and payload_log:
        import csv
        # Build header (timestamps, pose, velocity)
        header = ["sec","nsec","drone_id","p_w_x","p_w_y","p_w_z","v_w_x","v_w_y","v_w_z"]

        # Group metadata for dynamic columns
        try:
            _unwrapped = env.unwrapped
            _is_group = bool(getattr(_unwrapped, "group_mode", False)) or hasattr(_unwrapped, "_robots")
            _N = int(getattr(_unwrapped, "_num_robots", len(_unwrapped._robots))) if _is_group else 1
        except Exception:
            _is_group = False
            _N = 1

        # Determine observation dimensionality inferred from recorded rows
        try:
            _obs_dim = int(obs_dim_for_csv) if (obs_dim_for_csv is not None) else int(len(payload_log[0]) - 9 - (3 * (_N if _is_group else 0)) - 4)
        except Exception:
            _obs_dim = max(0, int(len(payload_log[0]) - 9 - (3 * (_N if _is_group else 0)) - 4))
        ...
        def _named_obs_header_dynamic():
            ...
            try:
                unwrapped = env.unwrapped
                group = bool(getattr(unwrapped, "group_mode", False)) or hasattr(unwrapped, "_robots")
            except Exception:
                group = False
            ...
            try:
                N = int(getattr(unwrapped, "_num_robots", len(unwrapped._robots)))
            except Exception:
                N = 1
            try:
                K = int(getattr(unwrapped, "_neighbors_k_eff", max(0, int(getattr(unwrapped.cfg, "neighbors_k", 2)))))
            except Exception:
                K = 0
            ...
            for i in range(N):
                names += [
                    f"r{i}_v_b_x", f"r{i}_v_b_y", f"r{i}_v_b_z",
                    f"r{i}_w_b_x", f"r{i}_w_b_y", f"r{i}_w_b_z",
                    f"r{i}_quat_w", f"r{i}_quat_x", f"r{i}_quat_y", f"r{i}_quat_z",
                    f"r{i}_prev_force", f"r{i}_prev_roll", f"r{i}_prev_pitch", f"r{i}_prev_yaw",
                ]
                for j in range(K):
        ...
        if len(names) != _obs_dim:
            names = [f"obs_{i}" for i in range(_obs_dim)]

                # Per-drone world positions (group mode)
        if _is_group and _N > 0:
            for i in range(_N):
                header += [f"r{i}_p_w_x", f"r{i}_p_w_y", f"r{i}_p_w_z"]

        header += names
        # Actions logged as [roll, pitch, force, yaw] (env 0 / first robot)
